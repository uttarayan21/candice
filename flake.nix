  ...
  outputs = {
    ...
    flake-utils.lib.eachDefaultSystem (
      system: let
        ...
        candice = craneLib.buildPackage (commonArgs
          // {
            inherit cargoArtifacts;
          });
      in {
        checks = {
          candice-clippy = craneLib.cargoClippy (commonArgs
            // {
              inherit cargoArtifacts;
              cargoClippyExtraArgs = "--all-targets -- --deny warnings";
            });
          candice-fmt = craneLib.cargoFmt {
            inherit src;
          };
          candice-nextest = craneLib.cargoNextest (commonArgs
            // {
              inherit cargoArtifacts;
              partitions = 1;
              partitionType = "count";
